...
from src.agents.MinerUTool import MinerUBboxExtractor

logger = logging.getLogger(__name__)


def local_image_to_data_url(path: str, max_pixels: Optional[int] = None) -> str:
    """
     Data URL (Base64) (max_pixels)
     max_pixels
    """
    ...
    try:
        ...
        logger.exception("Error encoding image: %s", path)
        return ""
...
class RAGAgent:
    ...
    def retrieve(self, sample: StandardSample) -> List[PageElement]:
        """
         PageElement 
        """
        logger.info("Retrieving sample qid=%s", sample.qid)
        try:
            ...
            logger.exception("Error during retrieval for qid=%s", sample.qid)
            return []

    def generate(self, query: str, retrieved_elements: List[PageElement]) -> Dict[str, Any]:
        """
         Query  Elements 
         'model_answer', 'messages', 'prompt_tokens', 'completion_tokens' 
        """
        #  self.system_prompt
        ...
        try:
            ...
            logger.exception("LLM API error during generation")
            model_answer = "Error during generation."

        return {
            "model_answer": model_answer,
            "messages": messages[1:], #  system prompt
    ...
    def build_context_message(self, elements: List[PageElement]) -> List[Dict[str, Any]]:
        """
         Context 
        Modified:  (page_xx.png) 
        Modified:  use_ocr_both  Model OCR  Raw OCR
        """
        content_list = []
            ...
            for i, el in enumerate(page_elements):
                ...
                if self.use_crop:
                    ...
                    if img_path and os.path.exists(img_path):
                        img_url = local_image_to_data_url(img_path, max_pixels=self.max_crop_pixels)
                        if img_url:
                ...
                if self.use_ocr and self.use_ocr_both:
                    ...
                    if text_content:
                        content_list.append({"type": "text", "text": f"**OCR Content**: {text_content}\n"})
    ...
    def process_sample(self, sample: StandardSample) -> StandardSample:
        ...
        sample.extra_info['model_answer'] = gen_result['model_answer']
        sample.extra_info['messages'] = gen_result['messages']

        logger.info("[Standard RAG Answer] %s", gen_result["model_answer"][:100])
        return sample
    ...
    def save_results(self, excel_path: str = "rag_results_summary.xlsx", json_path: str = "rag_results_summary.json"):
        ...
        for sample in self.loader.samples:
            model_ans = sample.extra_info.get('model_answer', "") if sample.extra_info else ""
        ...
        if excel_path:
            try:
                ...
                logger.info("Excel summary saved to: %s", excel_path)
            except ImportError:
                logger.error("pandas/openpyxl not installed; cannot save Excel output.")
            except Exception:
                logger.exception("Error saving Excel: %s", excel_path)
        ...
        if json_path:
            try:
                ...
                logger.exception("Error saving JSON: %s", json_path)
