...
class MMLongLoader(BaseDataLoader):
    """
    MMLongBench-Doc 
     MMLongBench-Doc  DocVQA  LLM 
    """
    
    def __init__(self, data_root: str, output_dir: str = "./", reranker: Optional[Qwen3VLReranker] = None, extractor: Optional[AgenticOCR] = None, judger: Optional[AgenticOCR] = None):
    ...
    def save_bad_cases(self, output_dir: str, task: str):
        ...
        if task in ["retrieval", "all"] and retrieval_bad_cases:
            ...
            for source, cases in retrieval_by_source.items():
                ...
                logger.info(f"   └─ Saved {len(cases)} cases for source '{source}' to {os.path.basename(p_src)}")
    ...
    def evaluate_retrieval(self) -> Dict[str, float]:
        """
         ...
         (avg_retrieved_page_count)
         Evidence Source  (Recall, Precision, Count)
        """
        total_metrics = collections.defaultdict(float)
        counts = collections.defaultdict(int)
        
        # : source -> metric_name -> value
        ...
        for sample in tqdm(self.samples, desc="Evaluating Retrieval"):
            ...
            #  Metrics
            #  Recall/Precision Gold Pages  UnanswerableGold Pages 
            ...
            if sample.gold_pages or is_unanswerable:
                ...
                if hasattr(self, '_compute_page_metrics'):
                     # _compute_page_metrics  gold_pages ( pred  P=1, R=1)
    ...
    def evaluate_generation(self, num_threads: int = 8) -> Dict[str, float]:
        ...
        1.  evidence_sources  model_eval ( Not Answerable)
        2.  input prompt token  output prompt token
        """
        total_metrics = collections.defaultdict(float)
        counts = collections.defaultdict(int)
        ...
        if self.llm_caller:
            logger.info("Using LLM-based Answer Extraction.")
        else:
            logger.warning("Warning: No LLM caller provided. Skipping Answer Extraction.")
            ...
            with ThreadPoolExecutor(max_workers=num_threads) as executor:
                ...
                for future in tqdm(as_completed(future_to_sample), total=len(self.samples), desc="Evaluating Generation"):
                    try:
                        ...
                        sample = future_to_sample[future] #  sample 
                        if count > 0:
    ...
    def load_data(self) -> None:
        ...
        logger.info(f"Loading MMLongBench data from: {self.json_path}")
        ...
        logger.info(f"✅ Successfully loaded {count} MMLongBench samples.")

    def _pdf_to_images(self, pdf_path: str) -> Dict[int, str]:
        if not os.path.exists(pdf_path):
             logger.warning(f"Warning: PDF not found at {pdf_path}")
             return {}
        ...
        try:
            ...
            logger.info(f"Converted {len(image_map)} pages.")
        except ImportError:
            logger.exception("Error: `pdf2image` library is not installed.")
            return {}
        except Exception as e:
            logger.exception(f"Error converting PDF {pdf_path}: {e}")
            return {}
        return image_map
    ...
    def pipeline(self, query: str, image_paths: List[str] = None, top_k: int = 10, trunc_thres=0.0, trunc_bbox=False) -> List[PageElement]:
        ...
        for page in target_pages:
            ...
            try:
                ...
                logger.exception(f"Error during agent execution on {img_path}: {e}")
...
if __name__ == "__main__":
    # Test code
    root_dir = "/path/to/MMLongBench-Doc"
    reranker_path = "/path/to/Qwen3-VL-Reranker-8B"

    #  Reranker 
    logger.info(f"⚡ Initializing Reranker instance from {reranker_path} ...")
    ...
    #  Loader 
    #  judger loader = MMLongLoader(..., judger=my_judger)
